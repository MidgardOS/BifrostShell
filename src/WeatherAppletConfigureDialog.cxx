...
WeatherAppletConfigureDialog::WeatherAppletConfigureDialog(WeatherApplet *parent) : QDialog(parent), applet(parent) {
    ui.setupUi(this);
    connect(ui.city, &QLineEdit::textEdited,
            [this](const QString &txt) {
                auto found = ui.cityList->findItems(txt, Qt::MatchStartsWith);
                if (found.count()) {
                    ui.cityList->setCurrentItem(found[0]);
                }
            }
    );
    ...
    connect(ui.cityList, &QTreeWidget::itemClicked,
            [this](QTreeWidgetItem *current) {
                ui.city->setText(current->text(0));
            }
    );
