...
WeatherApplet::WeatherApplet(QWidget *parent, const QString &theId) : DesktopApplet(parent, theId) {
...
void WeatherApplet::fetchData() {
    ...
    QString url = QString("http://api.openweathermap.org/data/2.5/weather?APPID=%1&units=%2&id=%3").arg(apiKey, units, cityId);
    ...
    url = QString("http://api.openweathermap.org/data/2.5/forecast?APPID=%1&units=%2&id=%3").arg(apiKey, units, cityId);
...
void WeatherApplet::setIcon(QLabel *label, const QString &icon) {
    QString cacheDir = QStandardPaths::writableLocation(QStandardPaths::CacheLocation) + "/api.openweathermap.org/";
    QDir dir;
    dir.mkpath(cacheDir);
    ...
    if (QFile::exists(filePath)) {
        ...
        KIO::StoredTransferJob *job = KIO::storedGet(
            QUrl("http://api.openweathermap.org/img/w/" + icon), KIO::Reload, KIO::HideProgressInfo
        );
...
ForecastWidget::ForecastWidget(QWidget *parent, bool showMinMax) : QWidget(parent) {
