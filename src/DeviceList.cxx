...
DeviceItem::DeviceItem(Solid::Device dev, const QVector<DeviceAction> &deviceActions) : device(dev) {
    ...
    if (storage) {
        ...
        connect(&mountBusyTimer, &QTimer::timeout, this, [this]() {
                mountButton->setVisible(! mountButton->isVisible());
            }
        );
        ...
        connect(mountButton, &QToolButton::clicked, mountButton, [this]() {
                statusLabel->hide();
    ...
    if (device.is<Solid::Battery>()) {
        ...
        connect(battery, &Solid::Battery::chargePercentChanged, this, [this, battery]() {
                statusLabel->setText(QString::number(battery->chargePercent()) + '%');
                chargeIcon->setPixmap(Battery::getStatusIcon(battery->chargePercent(),
                                      battery->chargeState() == Solid::Battery::Charging).pixmap(22));
            }
    ...
    for (const DeviceAction &action : deviceActions) {
        ...
        connect(button, &IconButton::clicked, button, [action, this]() {
...
DeviceItem::DeviceItem(const KdeConnect::Device &dev) {
    ...
    connect(configure, &QToolButton::clicked, configure, [this]() {
            if (! dialog ) {
    ...
    if (dev->plugins.contains("kdeconnect_sftp")) {
        ...
        connect(button, &IconButton::clicked, [dev]() {
                new KRun(QUrl(QLatin1String("kdeconnect://") + dev->id), nullptr);
            }
        );
...
DeviceList::DeviceList(QWidget *parent) : QScrollArea(parent) {
    ...
    connect(Solid::DeviceNotifier::instance(), &Solid::DeviceNotifier::deviceAdded, this, &DeviceList::deviceAdded);
    connect(Solid::DeviceNotifier::instance(), &Solid::DeviceNotifier::deviceRemoved, this, &DeviceList::deviceRemoved);
...
DeviceItem *DeviceList::addDevice(Solid::Device device) {
    ...
    if (device.is<Solid::Battery>() && 
        (device.as<Solid::Battery>()->type() != Solid::Battery::PrimaryBattery)) {
