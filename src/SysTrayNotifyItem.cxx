...
SysTrayNotifyItem::SysTrayNotifyItem(QWidget *parent, const QString &service, const QString &path) : QLabel("?", parent) {
...
void SysTrayNotifyItem::fetchDataReply(QDBusPendingCallWatcher *w) {
    ...
    */
    ...
    if (//!toolTip.icon.isEmpty() ||
        ...
        tip = QString("<html><center><b>%1</b><br>%2</center></html>").arg(toolTip.title).arg(toolTip.subTitle);
    }
...
void SysTrayNotifyItem::mouseReleaseEvent(QMouseEvent *event) {
    ...
    if (event->button() == Qt::LeftButton) {
        ...
        if (reply.isError() && (reply.error().type() == QDBusError::UnknownMethod) && ! menuPath.isEmpty()) {
            QDBusMessage msg = QDBusMessage::createMethodCall(
                                    dbus->service(),
                                    menuPath,
