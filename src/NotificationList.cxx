...
NotifyItem::NotifyItem(QWidget *parent,
                       ...
                       const QIcon &icon,
                       const QStringList &theActions) : QFrame(parent, POPUP_FLAGS),
                                                        id(theId),
                                                        appName(app),
                                                        summary(theSummary),
                                                        body(theBody),
        ...
        for (int i = 0; i < actions.count(); i++) {
            if (((i % 2) != 0) && ! actions[i].isEmpty()) { // id
                ...
                connect(button, &QPushButton::clicked, this, [this, key]() {
                        QDBusMessage msg = QDBusMessage::createSignal("/org/freedesktop/Notifications",
    ...
    textLabel->setText(QString("<html><center><b>%1</b></center><br>%2</html>").arg(title).arg(body));
...
NotificationList::NotificationList(QWidget *parent) : QWidget(parent) {
...
void NotificationList::addItem(uint id,
