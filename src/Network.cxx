...
Network::Network(QWidget *parent) : SysTrayItem(parent) {
...
void Network::checkState() {
    ...
    if (! NetworkManager::primaryConnection() || ! NetworkManager::primaryConnection()->connection()) {
        if (QIcon::hasThemeIcon("network-offline")) {
            setPixmap(
                origPixmap = QIcon::fromTheme("network-offline").pixmap(
                    size()
                )
            );
        } else {
            setPixmap(
                origPixmap = QIcon::fromTheme("network-disconnect").pixmap(
                    size()
                )
            );
        }
