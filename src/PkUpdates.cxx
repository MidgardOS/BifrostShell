...
PkUpdates::PkUpdates(QWidget *parent) : SysTrayItem(parent) {
    KConfig config;
    ...
    setToolTip(
        i18n("Next check: %1 ").arg(QDateTime::currentDateTime().addMSecs(updateTimer.interval()).toString(Qt::SystemLocaleShortDate))
    );
...
void PkUpdates::transactionError(PackageKit::Transaction::Error error, const QString &details) {
    ...
    setToolTip(
        i18n("Last check: %1\nError on checking for updates: %2", QDateTime::currentDateTime().toString(Qt::SystemLocaleShortDate), details)
    );

    KNotification::event(
        "update error", i18n("Software Update Error"), details, QIcon::fromTheme("dialog-error").pixmap(32), this
    );
...
void PkUpdates::createToolTip(bool notify) {
    ...
    if (tooltip.isEmpty()) {
        ...
        tooltip = i18n("<html>Last check: %1<br>%2</html>", QDateTime::currentDateTime().toString(Qt::SystemLocaleShortDate), tooltip);
        setToolTip(tooltip);
        ...
        if (notify) {
            KNotification::event(
                "updates available", i18n("Software Updates Available"), tooltip,
...
void PkUpdates::addItems(QString &tooltip, const QList<PackageData> &list) const {
    ...
    if (list.count() == 4) {
        // if there's just one more, show it directly instead of "1 more"
        count++;
    }
