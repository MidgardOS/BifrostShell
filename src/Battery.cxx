...
Battery::Battery(QWidget *parent) : SysTrayItem(parent) {
...
QString Battery::secsToHM(int secs) const {
    ...
    QString hourStr   = i18np("%1 hour", "%1 hours", hour);
    QString minuteStr = i18np("%1 minute", "%1 minutes", minute);
...
QIcon Battery::getStatusIcon(int charge, bool isCharging) {
    ...
    int percentage = qRound(charge / 20.0) * 20;
    ...
    if (percentage < 20) {
        ...
        iconName = QString("%1").arg(percentage, 3, 10, QLatin1Char('0'));
    }
...
void Battery::changed() {
    ...
    switch (battery->chargeState()) {
        case Solid::Battery::NoCharge: {
            tip = i18n("Not Charging");
            break;
        }
        case Solid::Battery::FullyCharged: {
            tip = i18n("Fully Charged");
            break;
        }
        case Solid::Battery::Charging: // falls through
    ...
    setPixmap(getStatusIcon(battery->chargePercent(), battery->chargeState() == Solid::Battery::Charging).pixmap(size()));
    setToolTip(tip);
