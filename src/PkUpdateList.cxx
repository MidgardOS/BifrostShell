...
PkUpdateListItem::PkUpdateListItem(QWidget *parent,
                                   PackageKit::Transaction::Info info,
                                   const PkUpdates::PackageData &data) : QWidget(parent), package(data) {
...
PkUpdateList::PkUpdateList(QWidget *parent) : QWidget(parent), restart(PackageKit::Transaction::RestartNone) {
...
QSize PkUpdateList::sizeHint() const {
    ...
    if (savedSize.isValid()) {
        ...
        s.setHeight(
            layout()->contentsMargins().top() + installButton->sizeHint().height() +
...
void PkUpdateList::installOne() {
    ...
    connect(transaction.data(), &PackageKit::Transaction::eulaRequired, this,
            [this](const QString &eulaID,
                   const QString &packageID,
                   const QString &vendor,
                   const QString &licenseAgreement) {
                if (QMessageBox::question(this,
                                          i18n("Accept EULA"),
                                          QString("%1\n%2\n%3").arg(packageID).arg(vendor).arg(licenseAgreement)
                                          ) == QMessageBox::Yes) {
    ...
    connect(transaction.data(), &PackageKit::Transaction::repoSignatureRequired, this,
            [this](const QString &packageID,
                ...
                if ( QMessageBox::question(this, i18n("Validate Signature"),
                                           QString("%1\n%2\n%3\n%4\n%5\n%6\n%7").arg(packageID).arg(repoName).arg(keyUrl).arg(keyUserid).arg(keyId).arg(keyFingerprint).arg(keyTimestamp)) == QMessageBox::Yes ) {
    ...
    connect(transaction.data(), &PackageKit::Transaction::finished, this,
            [item, this](PackageKit::Transaction::Exit status, uint runtime) {
                ...
                if (! installQ.isEmpty()) {
                    // might have been cancelled, q cleared
                    installQ.dequeue();
                }
